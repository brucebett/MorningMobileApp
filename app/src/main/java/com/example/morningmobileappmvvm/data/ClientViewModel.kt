...
class ClientViewModel (var navController: NavController,
        var context: Context){
    var authRepositor: AuthViewModel

    init {
        authRepositor=AuthViewModel(navController,context)
        if (!authRepositor.isloggedin()){
        ...
        storageReference.putFile(filePath).addOnCompleteListener {
            if(it.isSuccessful){
                storageReference.downloadUrl.addOnSuccessListener {
                    ...
                    Toast.makeText(context,"Client add successfuly", Toast.LENGTH_LONG).show()
